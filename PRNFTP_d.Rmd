...
meteo_data_hourly <- do.call(rbind, lapply(split(coordinates,as.factor(coordinates$env)), function(x)
{
  daily_single_ag <- get_power(
    ...
    pars = c("T2M"), ### to compute WangEngel or TT20; retrieve hourly temperature data from nasapower
    ## pars = c("Tmin", "Tmax"), ### to compute GDD; retrieve daily temperature data from nasapower
    dates = c(paste0(x$year,"-01-01"),paste0(x$year,"-12-31")), ## get the temperature values for the year corresponding to the env 
    temporal_api = "hourly") ### need hourly temperature data ## change this to "daily" if you want to calculate your thermal time in GDD
...
meteo_data_hourly_1 <- meteo_data_hourly %>%
  ...
  dplyr::ungroup()%>%
  dplyr::select(env, project, LON, LAT, YEAR, MO, DY, WangEngel) %>% # select columns to be passed according to step 2... WangEngel
  #dplyr::select(env, project, LON, LAT, YEAR, MO, DY, TT20) %>% # select columns to be passed according to step 2... TT20
  #dplyr::select(env, project, LON, LAT, YEAR, MO, DY, GDD) %>% # select columns to be passed according to step 2... GDD
